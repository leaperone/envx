...
async function installFakeBrowser(cwd) {
...
fetch('http://127.0.0.1:' + port + '/callback?code=browser-code').catch(() => process.exit(1));
`
  );
...
test('push applies the selected organization identity to a new namespace', async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ currentOrg: 'team', currentOrgId: 'org-1' })
  );
...
test('device login keeps Dashboard authorization and API verification on distinct origins', async () => {
  ...
  const api = await listen((request, response) => {
    ...
    request.on('end', () => {
      apiRequests.push({ url: request.url, headers: request.headers, body: body ? JSON.parse(body) : null });
      response.writeHead(200, { 'Content-Type': 'application/json' });
